...
?>
	<div class="wrap">
		...
		<h2 class="nav-tab-wrapper">
			...
			<a href="?page=disable-bloat&tab=data" class="nav-tab <?php echo esc_attr($active_tab) == 'data' ? 'nav-tab-active' : ''; ?>"><?php esc_attr_e('Plugin data', 'disable-dashboard-for-woocommerce'); ?></a>
		...
		<div class="disable-bloat-container">
			<form action="options.php" method="POST">
				<?php if ($active_tab == 'main') {
					...
					settings_fields('wcbloat-data-options');
					do_settings_sections('wcbloat-data');
				}
				submit_button(); ?>
			</form>
		</div>
...
function wcbloat_woo_not_installed_info_bar()
{
	...
	if (!wcbloat_is_woo_active()) {
		$woo_not_installed_bar =  sprintf('<p class="wcbloat-bar">' . __('You need to <strong>activate WooCommerce</strong> to make the options below effective.', 'disable-dashboard-for-woocommerce') . '<hr /></p>');
...
{
	$is_free_plan = wcbloat_fs()->is_free_plan();
	/* translators: link to external plugin homepage */
	$buy_pro_bar = $is_free_plan ? sprintf('<p class="wcbloat-bar">' . __('Get <a href="%s" target="_blank">Disable Bloat for WordPress & WooCommerce PRO</a> to unlock the<span class="wcbloat-pro-label">PRO</span>options.', 'disable-dashboard-for-woocommerce'), 'https://disablebloat.com/?utm_source=settings&utm_medium=referral&utm_campaign=Buy+PRO+Bar') : '';
...
require_once 'class-disable-bloat-settings-plugin-data.php';
